...
const ProjectLogo = ({ project }: { project: ProjectLogo }) => {
  ...
  if (!hasLogo || imageError) {
    return <ProjectLogoLetter size="48px" name={project.name} />
  }

  return <img src={project.logoUrl} alt={project.name} className="bg-coolgray-800 h-12 w-12 rounded-full object-cover" onError={() => setImageError(true)} />
}
      ...
      {projectLogos.length > 0 && (
        <div className="mb-3 flex items-center gap-1">
          <div className="flex -space-x-2">
            {projectLogos.slice(0, 5).map((project) => (
              <ProjectLogo key={project.id} project={project} />
            ))}
          </div>
