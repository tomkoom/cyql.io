...
export const formatIdShort = (str: string): string => {
  return str.substring(0, 2) + "..." + str.substring(str.length - 3)
}

export const formatIdNormal = (str: string): string => {
...
export const formatDiscord = (url: string): string => {
  const formattedURL = url.includes("discord.gg/") ? url.split("discord.gg/")[1] : url.includes("discord.com/") ? url.split("discord.com/")[1] : url
  return formattedURL.length > 12 ? `${formattedURL.substring(0, 12)}…` : formattedURL
}
